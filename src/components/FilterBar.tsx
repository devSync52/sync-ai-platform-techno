...
export default function FilterBar({ placeholder, searchTerm, onSearch, filters }: FilterBarProps) {
  ...
  return (
    <div className="bg-white p-4 rounded-xl shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <div className="flex-1 flex flex-col sm:flex-row sm:items-center gap-2 sm:justify-end">
        ...
        <Button onClick={() => setShowFilters(!showFilters)} className="sm:hidden flex items-center gap-1 text-sm text-white border border-gray-300 rounded px-3 py-2">
        ...
        <div className={`flex flex-wrap gap-2 ${showFilters ? 'block' : 'hidden sm:flex'}`}>
          {
            filters.map((filter, index) => {
              ...
              return (
                <select key={index} value={filter.value} onChange={(e) => filter.onChange(e.target.value)} className="border border-gray-300 rounded px-2 py-2 text-sm bg-white">
                  {
