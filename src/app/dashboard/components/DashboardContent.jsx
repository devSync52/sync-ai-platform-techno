        ...
        <section className="min-h-0 flex-1 overflow-y-auto px-4 pb-8 pt-7 xl:px-9">
            <div className="mx-auto">
                ...
                <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
                    <DashboardPanel title="Delivery Performance" action="View KPI" href={"/dashboard/sla-kpi"}>
                        <PerformanceChart data={performanceData} />
                    </DashboardPanel>

                    <DashboardPanel title="Discrepancy Types" action="View" href={"/dashboard/discrepancies"}>
                        <DiscrepancyDonut data={initialDiscrepancyTypes} />
                    </DashboardPanel>
                </div>

                <div className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
                    <DashboardPanel title="Carrier On-Time Rate">
...
function DashboardPanel({ title, action, children, href }) {
    return (
        <section className="interactive-card motion-scale-in rounded-xl border border-[#ece8f2] bg-white/95 p-6 shadow-[0_1px_3px_rgba(19,12,35,0.08)] backdrop-blur">
            <div className="mb-6 flex items-center justify-between">
                ...
                {
                    action && (
                        <Link className="flex items-center gap-2 text-xs font-medium text-[#5f5876] transition hover:text-[#6700e7]" href={href}>
                            {action}
