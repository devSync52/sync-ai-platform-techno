...
import {
  normalizeIntegrationTypes,
  resolveOperationalOnlySource,
} from '@/lib/integrations/operationalSource'
...
export default async function OrdersPage() {
  ...
  if (userAccountId) {
    const { data: userAccountRecord } = await supabase
      .from('accounts')
    ...
    if (isClientLikeRole) {
      ...
      const { data: principalAccountRecord } = await supabase
        .from('accounts')
        .select('id,parent_account_id')
        .eq('id', principalAccountId)
        .maybeSingle()

      integrationAccountId = principalAccountRecord?.parent_account_id ?? principalAccountId
    }

    const { data: parentIntegrations } = await supabase
