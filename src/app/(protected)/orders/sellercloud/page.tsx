...
export default function DashboardSellercloud() {
  ...
  const cards = [
    { title: 'Orders', icon: <Package size={20} />, value: orders.length, color: 'text-blue-600' },
    { title: 'Sales', icon: <DollarSign size={20} />, value: 'R$ 0,00', color: 'text-green-600' },
    { title: 'Inventory', icon: <Boxes size={20} />, value: 0, color: 'text-yellow-600' },
    { title: 'In Transit', icon: <Truck size={20} />, value: 0, color: 'text-purple-600' }
  ]
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-4 sm:p-6 space-y-10">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-xl font-bold">Sellercloud Orders</h2>
      ...
      <div className="overflow-x-auto bg-white rounded-xl shadow-sm">
        <table className="min-w-full text-sm">
          <thead className="bg-gray-50 text-gray-600">
            <tr>
              <th className="py-3 px-4 text-left font-medium cursor-pointer" onClick={() => handleSort('order_id')}>
                Order ID {renderSortIcon('order_id')}
              </th>
              <th className="py-3 px-4 text-left font-medium">Order Date</th>
              <th className="py-3 px-4 text-left font-medium cursor-pointer" onClick={() => handleSort('marketplace')}>
                Marketplace {renderSortIcon('marketplace')}
              </th>
              <th className="py-3 px-4 text-left font-medium cursor-pointer" onClick={() => handleSort('status')}>
                Status {renderSortIcon('status')}
              </th>
              <th className="py-3 px-4 text-left font-medium cursor-pointer" onClick={() => handleSort('total_amount')}>
          ...
          <tbody className="divide-y divide-gray-200">
            {paginatedOrders.map((order, index) => (
              <tr key={index} className="hover:bg-gray-50">
                <td className="py-3 px-4 font-medium text-gray-800">{order.order_id}</td>
                <td className="py-3 px-4 text-gray-500">{order.order_date?.split('T')[0]}</td>
                <td className="py-3 px-4 text-gray-600">{order.marketplace}</td>
                ...
                <td className="py-3 px-4 text-gray-800">R$ {order.total_amount?.toFixed(2)}</td>
