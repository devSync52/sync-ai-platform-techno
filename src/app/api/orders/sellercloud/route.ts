...
const ENCRYPTION_KEY = "SYNC_SECRET";
...
function parseCredentials(raw: unknown): SellercloudCredentials | null {
  ...
  if (typeof raw === "string") {
    // Try plain JSON first (for unencrypted storage)
    try {
      parsed = JSON.parse(raw);
    } catch {
      // If not plain JSON, try encrypted payload
      try {
...
function ensureRequiredName(firstRaw: any, lastRaw: any, emailRaw?: any) {
  ...
  if (first && !last)
    return { firstName: first, lastName: first || "Customer" };
  if (!first && last) return { firstName: last, lastName: last };
  if (emailLocalPart)
    return { firstName: emailLocalPart, lastName: emailLocalPart };
...
function mapDraftToSellercloudOrder(draft: any, body: any) {
  ...
  const products = items
    ...
    .map((item) => {
      ...
      const sourceProductId = String(item.sku ?? item.itemId ?? "").trim();
      const normalizedProductId =
        normalizeSellercloudProductId(sourceProductId);
      const referenceId = String(item.reference_id ?? "").trim();
      return {
        ProductID: normalizedProductId || sourceProductId,
        ReferenceID:
          referenceId ||
          (normalizedProductId !== sourceProductId ? sourceProductId : ""),
  ...
  return {
    ...
    OrderDetails: {
      ...
      OrderSourceOrderID: String(
        body?.orderSourceOrderId ?? draft?.id ?? "",
      ).trim(),
      DisableInventoryCount: Boolean(body?.disableInventoryCount ?? false),
      OrderDate: String(
        body?.orderDate ?? draft?.created_at ?? new Date().toISOString(),
      ),
      EbaySellingManagerSalesRecordNumber: String(
        body?.ebaySellingManagerSalesRecordNumber ?? "",
      ).trim(),
    },
    GiftDetails: {
...
async function fetchSellercloudJson(
  baseUrl: string,
  token: string,
  path: string,
): Promise<any> {
...
async function loadSellercloudContext() {
  ...
  if (
    ...
    return {
      error: "Sellercloud integration is not active for this account",
      status: 400 as const,
    };
  }
  ...
  try {
    ...
    if ("error" in ctx) {
      return NextResponse.json(
        { error: ctx.error, details: (ctx as any).details ?? null },
        { status: ctx.status },
      );
    }
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          error: `Failed to load Sellercloud warehouses (${response.status})`,
          details: json || text,
        },
        { status: 400 },
      );
    ...
    const warehouses = rows
      .map((row: any) => ({
        ...
        name: String(
          row?.Name ?? row?.WarehouseName ?? row?.name ?? "Unnamed Warehouse",
        ),
        raw: row,
      }))
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!Array.isArray(payload.Products) || payload.Products.length === 0) {
      return NextResponse.json(
        { error: "Order must include at least one product line" },
        { status: 400 },
      );
    }
    ...
    if (!resolvedShipFromWarehouseId) {
      const publicWarehouseId = String(
      ...
      if (publicWarehouseId) {
        const { data: warehouseRow } = await ctx.admin
          ...
          .maybeSingle();
        resolvedShipFromWarehouseId = toPositiveInt(
          (warehouseRow as any)?.wms_facility_id,
        );
      }
    }
    ...
    if (resolvedShipFromWarehouseId) {
      for (const product of payload.Products || []) {
        ...
        if (!current)
          (product as any).ShipFromWareHouseID = resolvedShipFromWarehouseId;
      }
    }
    ...
    if (!resolvedCompanyId) {
      const whJson = await fetchSellercloudJson(
        baseUrl,
        token,
        "/rest/api/Warehouses",
      );
      ...
      const matchedWarehouse = warehouses.find((w: any) => {
        ...
        return (
          !!resolvedShipFromWarehouseId && id === resolvedShipFromWarehouseId
        );
      });
    ...
    if (!resolvedCompanyId) {
      ...
      const preferred = companies.find((c: any) =>
        Boolean(c?.IsDefault ?? c?.isDefault),
      );
      const picked = preferred || companies[0];
      resolvedCompanyId = toPositiveInt(
        (picked as any)?.ID ?? (picked as any)?.Id ?? (picked as any)?.id,
      );
    }
    ...
    if (!response.ok) {
      ...
      const duplicateSourceIdError =
        String(message)
          .toLowerCase()
          .includes("same order source order id already exists") ||
        String(message)
          .toLowerCase()
          .includes("order source order id already exists") ||
        rawErrorText
          .toLowerCase()
          .includes("same order source order id already exists") ||
        rawErrorText
      ...
      if (duplicateSourceIdError) {
        ...
        if (persistError) {
          console.warn(
            "[orders/sellercloud] duplicate order detected but failed to persist success status",
            {
              draftId,
